  ...
  try {
    ...
    if (existing && existing.rawApiData) {
      ...
      const cached = {
        ...
        destinationBranch: existing.commerce?.destinationBranch || raw.destinationBranch || null,
        originBranch: raw.originBranch || existing.commerce?.originBranch || null,
        deliveryChargeBreakdown: raw.deliveryChargeBreakdown || existing.commerce?.deliveryChargeBreakdown || null,
        shippingAmount: raw.shippingAmount ?? existing.commerce?.shippingAmount ?? 0,
    ...
    if (richData) {
      ...
      const decodedResponse = {
        ...
        destinationBranch: richData.destinationBranch || existing?.commerce?.destinationBranch || null,
        originBranch: richData.originBranch || existing?.commerce?.originBranch || null,
        deliveryChargeBreakdown: richData.deliveryChargeBreakdown || existing?.commerce?.deliveryChargeBreakdown || null,
        shippingAmount: richData.shippingAmount ?? existing?.commerce?.shippingAmount ?? 0,
    ...
    const fallback = {
      ...
      originBranch: existing.commerce?.originBranch || null,
      destinationBranch: existing.commerce?.destinationBranch || null,
      deliveryChargeBreakdown: existing.commerce?.deliveryChargeBreakdown || null,
      shippingAmount: existing.commerce?.shippingAmount || 0,
      customerProfile: {
        name: typeof existing.customer === 'object' && existing.customer ? existing.customer.name : existing.customer,
        phone: existing.customer?.phone || existing.customerPhone,
      },
